...
async def metrics_broadcaster():
    ...
    while True:
        try:
            ...
            # Atomic Get and Reset (stats:live:...)
            pipe = r.pipeline()
            pipe.getset("stats:live:positive", 0)
            pipe.getset("stats:live:negative", 0)
            pipe.getset("stats:live:neutral", 0)
            results = await pipe.execute()
            
            pos = int(results[0]) if results[0] else 0
            neg = int(results[1]) if results[1] else 0
            neu = int(results[2]) if results[2] else 0
