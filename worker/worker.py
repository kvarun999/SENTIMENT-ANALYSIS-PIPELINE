...
class SentimentWorker:
    ...
    async def update_redis_stats(self, sentiment_label):
        timestamp = datetime.utcnow().strftime('%Y-%m-%dT%H:%M')
        await self.redis.incrby(f"stats:{timestamp}:{sentiment_label}", 1)
        await self.redis.incrby(f"stats:live:{sentiment_label}", 1)
    ...
    async def process_message(self, msg_id, data):
        try:
            ...
            await self.update_redis_stats(result['sentiment_label'])
            await self.redis.xack(REDIS_STREAM, REDIS_GROUP, msg_id)
